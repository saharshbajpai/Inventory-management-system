...
def test_create_product_invalid_price_and_stock(client):
    payload = {
    ...
    response = client.post("/api/products", json=payload2)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_get_products(client):
    ...
    client.post("/api/products", json=p1)
    client.post("/api/products", json=p2)
...
def test_get_product_by_id(client):
    ...
    response_missing = client.get("/api/products/00000000-0000-0000-0000-000000000000")
...
def test_update_product(client):
    ...
    response = client.put(f"/api/products/{prod_id}", json=update_payload)
    assert response.status_code == status.HTTP_200_OK
...
def test_delete_product(client):
    ...
    response_get = client.get(f"/api/products/{prod_id}")
